...
export async function findClosetChart(): Promise<{ repo: string; version: string }> {
  ...
  throw new Error('No repository provides the "closet" chart — add https://rancher.github.io/ui-internal-tools/ai/magic-closet/ in Apps → Repositories');
}
...
// k8s Secret in the barn-secrets namespace, labeled with the owning
// Rancher user so sets are scoped per-user. One set can be the default.

export const SECRETS_NS = 'barn-secrets';

const LBL_KIND = 'barn.io/kind';
const LBL_OWNER = 'barn.io/owner';
const LBL_DEFAULT = 'barn.io/default';
const ANN_NAME = 'barn.io/display-name';
...
// Editor content pod
//
// A tiny nginx Deployment+Service (content in a ConfigMap) that serves the
// LEFT/RIGHT iframe pages for the Editor page. It's created idempotently when
// the extension loads (create-if-missing), so it exists without a separate
// install step. It lives in the `local` cluster so the global button works from
// any context (including non-cluster pages), and the iframes load it
// same-origin through the Kubernetes API service proxy.
// ---------------------------------------------------------------------------
const EDITOR_CLUSTER = 'local';
const EDITOR_NS = 'barn';
const EDITOR_NAME = 'barn-editor';
const EDITOR_BASE = `/k8s/clusters/${ EDITOR_CLUSTER }`;

// Same-origin service-proxy URL to a file the content pod serves (e.g. left.html).
export function editorContentUrl(file: string): string {
  return `${ EDITOR_BASE }/api/v1/namespaces/${ EDITOR_NS }/services/http:${ EDITOR_NAME }:80/proxy/${ file }`;
}

function editorPage(title: string): string {
  // Minimal standalone page; big centered label so the iframe content is obvious.
  return `<!doctype html><html><head><meta charset="utf-8"><title>${ title }</title>` +
    `<style>html,body{height:100%;margin:0}body{display:flex;align-items:center;justify-content:center;` +
    `font-family:system-ui,sans-serif;font-size:15vmin;font-weight:700;color:#6c6c76;background:#fff}` +
    `@media (prefers-color-scheme:dark){body{color:#b6b6c2;background:#1b1c21}}</style>` +
    `</head><body>${ title }</body></html>`;
}

// GET a Steve resource; resolve null if it doesn't exist yet.
function editorGet(type: string): Promise<any> {
  return rancherFetch(`${ EDITOR_BASE }/v1/${ type }/${ EDITOR_NS }/${ EDITOR_NAME }`).catch(() => null);
}

let ensureEditorInFlight: Promise<void> | null = null;

// Create the namespace + content ConfigMap + nginx Deployment + Service if they
// aren't there yet. Idempotent and safe to call repeatedly (dedup'd in-flight);
// swallows errors (e.g. the user lacks create rights) so it never breaks a page.
export function ensureEditorContent(): Promise<void> {
  if (ensureEditorInFlight) {
    return ensureEditorInFlight;
  }

  ensureEditorInFlight = (async() => {
    // Namespace
    const ns = await rancherFetch(`${ EDITOR_BASE }/v1/namespaces/${ EDITOR_NS }`).catch(() => null);

    if (!ns) {
      await rancherFetch(`${ EDITOR_BASE }/v1/namespaces`, {
        method: 'POST',
        body:   JSON.stringify({ apiVersion: 'v1', kind: 'Namespace', metadata: { name: EDITOR_NS } }),
      }).catch(() => null);
    }

    // Content ConfigMap (upsert, so the LEFT/RIGHT content can change on redeploy)
    const cmBody = {
      apiVersion: 'v1',
      kind:       'ConfigMap',
      metadata:   { namespace: EDITOR_NS, name: EDITOR_NAME },
      data:       { 'left.html': editorPage('LEFT'), 'right.html': editorPage('RIGHT') },
    };
    const cm = await editorGet('configmaps');

    if (cm) {
      await rancherFetch(`${ EDITOR_BASE }/v1/configmaps/${ EDITOR_NS }/${ EDITOR_NAME }`, { method: 'PUT', body: JSON.stringify({ ...cm, data: cmBody.data }) }).catch(() => null);
    } else {
      await rancherFetch(`${ EDITOR_BASE }/v1/configmaps`, { method: 'POST', body: JSON.stringify(cmBody) }).catch(() => null);
    }

    // Deployment (nginx serving the ConfigMap)
    if (!await editorGet('apps.deployments')) {
      await rancherFetch(`${ EDITOR_BASE }/v1/apps.deployments`, {
        method: 'POST',
        body:   JSON.stringify({
          apiVersion: 'apps/v1',
          kind:       'Deployment',
          ...
          spec:       {
            replicas: 1,
            selector: { matchLabels: { app: EDITOR_NAME } },
            template: {
              metadata: { labels: { app: EDITOR_NAME } },
              spec:     {
                containers: [{
                  name:         'nginx',
                  image:        'nginx:alpine',
                ...
                volumes: [{ name: 'content', configMap: { name: EDITOR_NAME } }],
              },
            },
          },
        }),
    ...
    // Service (ClusterIP :80 — reached by the iframes via the service proxy)
    if (!await editorGet('services')) {
      await rancherFetch(`${ EDITOR_BASE }/v1/services`, {
        method: 'POST',
        body:   JSON.stringify({
          apiVersion: 'v1',
          kind:       'Service',
          metadata:   { namespace: EDITOR_NS, name: EDITOR_NAME },
          spec:       { selector: { app: EDITOR_NAME }, ports: [{ port: 80, targetPort: 80 }] },
        }),
      }).catch(() => null);
    }
  })().finally(() => {
    ensureEditorInFlight = null;
  });
