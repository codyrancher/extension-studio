...
// Params never shown as closet config: secret-set-managed credentials.
const HIDDEN_PARAMS = [
  'ghToken', 'awsAccessKey', 'awsSecretKey', 'apiKey',
...
export default {
  ...
  data() {
    return {
      ...
      connect:   null,     // open connect modal: the MCP-server sidecar object
      share:     null,     // open share modal: { name, url, logins, loading, error }
      copiedKey: '',       // which share value's Copy button just fired
  ...
  computed: {
    ...
    vscodeCommand() {
      ...
      return [
        ...
        `sed -i.bak "/# >>> barn $H >>>/,/# <<< barn $H <<</d" "$HOME/.ssh/config"`,
  ...
  methods: {
    ...
    launchLink(s) {
      if (s.launch === false || s.status !== 'running') {
        return null; // headless service (e.g. MCP server) — nothing to browse
      }
    ...
    canInternalLaunch(s) {
      ...
      return !!(s.launch !== false && s.internal && s.status === 'running' && s.name !== 'rancher-browser' && b && b.status === 'running');
    },
    ...
    // Share: open a modal with the address + each login. Passwords are fetched
    // on demand from the closet api, never in the sidecar list.
    async shareSidecar(s) {
      this.share = { name: s.name, url: '', logins: [], loading: true, error: '' };
      try {
        const d = await rancherFetch(`${ this.apiBase }/sidecars/${ s.name }/share`);

        this.share = { name: s.name, url: d.url, logins: d.logins || [], loading: false, error: '' };
      } catch (e) {
        this.share = { name: s.name, url: '', logins: [], loading: false, error: e.message };
      }
    },
    ...
    // Copy one share value; `key` drives the per-button "Copied" feedback.
    ...
    // The three lines for one login, to hand to someone.
    shareBlock(login) {
      return `${ this.share.url }\nuser: ${ login.user }\npass: ${ login.pass }`;
    },

    // Clipboard write with a fallback for insecure (plain-http) origins; throws
    // if both paths fail so the caller can surface the error.
    async copyText(text) {
      ...
      document.body.removeChild(ta);
      if (!ok) {
        throw new Error('clipboard blocked (needs https or localhost)');
      }
    },
...
<template>
  <div class="closet-dashboard">
        ...
        >
          ...
          <template #links>
            ...
            <a
              ...
              @click.prevent="shareSidecar(s)"
            >Share</a>
            <a
              v-if="s.mcpCommand"
              href="#"
              @click.prevent="openConnect(s)"
            >Connect</a>
          </template>
    ...
    >
      <div class="share-modal">
        ...
        <div class="share-modal__body">
          ...
          <div
            v-else-if="share.error"
            class="share-hint err"
          >
            {{ share.error }}
          </div>
          <template v-else>
            <div class="share-field">
              <span class="share-label">address</span>
              <span class="share-val">{{ share.url }}</span>
              <RcButton
                variant="secondary"
                size="small"
                @click="copyValue(share.url, 'addr')"
              >
                {{ copiedKey === 'addr' ? 'Copied' : 'Copy' }}
              </RcButton>
            </div>
            <div
              v-for="(l, i) in share.logins"
              :key="i"
              class="share-login"
            >
              <div class="share-field">
                <span class="share-label">user</span>
                <span class="share-val">{{ l.user }}</span>
                <RcButton
                  variant="secondary"
                  size="small"
                  @click="copyValue(l.user, `u${i}`)"
                >
                  {{ copiedKey === `u${i}` ? 'Copied' : 'Copy' }}
                </RcButton>
              </div>
              <div class="share-field">
                <span class="share-label">pass</span>
                <span class="share-val">{{ l.pass }}</span>
                <RcButton
                  variant="secondary"
                  size="small"
                  @click="copyValue(l.pass, `p${i}`)"
                >
              ...
              <RcButton
                variant="primary"
                size="small"
                @click="copyValue(shareBlock(l), `s${i}`)"
              >
                {{ copiedKey === `s${i}` ? 'Copied' : 'Share' }}
              </RcButton>
            </div>
          </template>
        </div>
    ...
    >
      <div class="share-modal">
        <div class="share-modal__head">
          <span>{{ connect.name }} — connect</span>
          <RcButton
            variant="secondary"
            size="small"
            @click="closeConnect"
        ...
        <div class="share-modal__body">
          <div class="share-hint">
            Run this in the Claude CLI (in the workspace) to enable the MCP server:
          </div>
          <div class="mcp-cmdrow">
            <code class="mcp-cmd">{{ connect.mcpCommand }}</code>
            <RcButton
              variant="secondary"
              size="small"
              @click="copyValue(connect.mcpCommand, 'mcp')"
            >
...
/* Copyable `claude mcp add` command shown on MCP-server sidecar cards (figma).
   Lives in SidecarCard's #links slot but styled from the parent scope (slot
   content carries the parent's scope), taking a full row via flex-basis. */
.mcp-block { flex-basis: 100%; margin-top: 4px; }
.mcp-cmdrow { display: flex; align-items: center; gap: 8px; }
.mcp-cmd {
  flex: 1;
  min-width: 0;
  overflow-x: auto;
...
.mcp-hint { margin-top: 4px; font-size: 12px; color: var(--muted); }
...
.share-modal {
  ...
  .share-hint.err { color: var(--error); }
}
</style>
