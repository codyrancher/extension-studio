...
asyncio.run(main())
PYEOF
`,ye=String.raw`#!/usr/bin/with-contenv bash
...
asyncio.run(run())
PYEOF
`;function ve(){return{apiVersion:"apps/v1",kind:"Deployment",metadata:{namespace:d,name:pe,labels:{app:pe}},spec:{replicas:1,selector:{matchLabels:{app:pe}},strategy:{type:"Recreate"},template:{metadata:{labels:{app:pe}},spec:{containers:[{name:"browser",image:me,ports:[{name:"http",containerPort:fe},{name:"cdp",containerPort:ge}],env:[{name:"PUID",value:"1000"},{name:"PGID",value:"1000"},{name:"CUSTOM_PORT",value:`${fe}`},{name:"TITLE",value:"Barn"},{name:"NODE_IP",valueFrom:{fieldRef:{fieldPath:"status.hostIP"}}},{name:"CHROME_CLI",value:`https://$(NODE_IP)${A(m)} --no-first-run --start-maximized --disable-infobars --disable-session-crashed-bubble --hide-crash-restore-bubble --disable-backgrounding-occluded-windows --disable-renderer-backgrounding --disable-background-timer-throttling --allow-insecure-localhost --ignore-certificate-errors --remote-debugging-port=${ge} --allow-running-insecure-content`}],volumeMounts:[{name:"dshm",mountPath:"/dev/shm"},{name:"services",mountPath:"/custom-services.d/cdp-proxy",subPath:"cdp-proxy",readOnly:!0},{name:"services",mountPath:"/custom-services.d/stream-keepalive",subPath:"stream-keepalive",readOnly:!0}],readinessProbe:{httpGet:{path:"/json/version",port:ge},periodSeconds:10}}],volumes:[{name:"dshm",emptyDir:{medium:"Memory",sizeLimit:"1Gi"}},{name:"services",configMap:{name:we,defaultMode:365}}]}}}}}let ke=null;function Ee(){return{apiVersion:"v1",kind:"ConfigMap",metadata:{namespace:d,name:we,labels:{app:pe}},data:{"cdp-proxy":be,"stream-keepalive":ye}}}const xe=[{name:"http",port:fe,targetPort:"http"},{name:"cdp",port:ge,targetPort:"cdp"}];async function Te(){const e=await N("configmaps",we);e&&await(0,a.c)(`${f}/v1/configmaps/${d}/${we}`,{method:"PUT",body:JSON.stringify({...e,data:Ee().data})}).catch(()=>null)}function Se(){return ke||(ke=(async()=>{await he(),await Z({type:"configmaps",namespace:d,name:we,body:Ee}),await Te(),await Z({type:"apps.deployments",namespace:d,name:pe,body:ve}),await Z({type:"services",namespace:d,name:pe,body:()=>ae(pe,xe)})})().finally(()=>{ke=null}),ke)}const Ae=24e4;async function Re(e,n,t=Ae,a){return $e(`/v1/pods/${(0,s.e0)(e)}/exec`,{command:n,timeoutMs:t,...a?{container:a}:{}})}async function $e(e,n){try{const t=await(0,s.Wp)(e,n);return{stdout:t.stdout||"",stderr:t.stderr||"",status:t.status||"",code:"number"===typeof t.code?t.code:-1,transport:!!t.transport}}catch(t){return{stdout:"",stderr:"",status:t?.message||String(t),code:-1,transport:!0}}}async function Oe(e,n,t,a){return(await Re(e,n,t??Ae,a)).stdout}class Ne extends Error{constructor(e,n){const t=n.stderr.trim()||n.stdout.trim().split("\n").slice(-3).join(" ").trim()||n.status.trim(),a=n.transport?"could not be run in the pod":`failed in the pod (exit ${n.code})`;super(`${e} ${a}: ${t.slice(0,400)||"it said nothing at all"}`),r(this,"code",void 0),r(this,"stdout",void 0),r(this,"stderr",void 0),r(this,"status",void 0),this.name="PodExecError",this.code=n.code,this.stdout=n.stdout,this.stderr=n.stderr,this.status=n.status}}function _e(e){const n=`export HOME=/app/.home; ${e}`;return["/bin/sh","-c",`setpriv --reuid=1000 --regid=1000 --init-groups /bin/sh -c ${ft(n)}`]}async function Ie(e,n){return(await Pe(e,n)).stdout}function Pe(e,n){return $e(`/v1/extensions/${(0,s.e0)(e)}/exec`,{script:n})}async function je(e,n,t){const a=await Pe(e,n);if(0!==a.code)throw new Ne(t,a);return a.stdout}const Ce=new Map;function Me(e){if(!Ce.has(e)){const n=De(e).catch(n=>{throw Ce.delete(e),n});Ce.set(e,n)}return Ce.get(e)}async function De(e){await Ie(e,["test -d .git && exit 0",'printf "node_modules/\\n" > .gitignore',"git init -q -b main","git add -A",'git -c user.email=barn@rancher.local -c user.name=barn commit -q -m "The seeded extension"'].join(" ; "))}function Be(e,n){return Ie(e,n)}async function He(e){const n=await Ie(e,"find . -name node_modules -prune -o -name .git -prune -o -type f -print | sed 's|^\\./||'");return n.split("\n").map(e=>e.trim()).filter(Boolean).sort()}async function Le(e,n,t=40){if(!n.trim())return[];const a=await Ie(e,`grep -rnF --exclude-dir=node_modules --exclude-dir=.git -- ${ft(n)} . 2>/dev/null | head -n ${t}`).catch(()=>"");return a.split("\n").map(e=>e.trim()).filter(Boolean).map(e=>{const n=/^\.?\/?(.+?):(\d+):(.*)$/.exec(e);return n?{path:n[1],line:parseInt(n[2],10),text:n[3].trim()}:null}).filter(Boolean)}async function Ue(e,n){return Ie(e,`cat ${ft(n)} 2>/dev/null`)}async function We(e,n,t){const a=await dt().catch(()=>""),s=`/app/.attachments/pick-${Date.now().toString(36)}.png`,o=await Ie(e,["mkdir -p /app/.attachments ;",'node "$HOME/.claude/skills/barn-screenshot/screenshot.mjs"',`--path ${ft(n||"/")}`,`--note ${ft(`${t}=picked`)}`,a?`--token ${ft(a)}`:"",`--output ${s} >/dev/null 2>&1 || true ;`,`[ -f ${s} ] && printf %s BARN-PICK: && base64 -w0 ${s} && echo ;`,`[ -f ${s}.json ] && printf %s BARN-PICK-REGIONS: && base64 -w0 ${s}.json && echo ;`,`rm -f ${s}.json ;`,"true"].filter(Boolean).join(" ")).catch(()=>""),r=e=>{const n=o.indexOf(e);return-1===n?"":o.slice(n+e.length).split("\n")[0].trim()},i=r("BARN-PICK:");if(!i)return{src:"",path:"",region:null};let h=null;try{const e=r("BARN-PICK-REGIONS:"),n=e?JSON.parse(atob(e)):null,t=Array.isArray(n?.regions)?n.regions[0]:null;t&&Number(t.width)>0&&Number(t.height)>0&&(h={x:Number(t.x)||0,y:Number(t.y)||0,width:Number(t.width),height:Number(t.height)})}catch{h=null}return{src:`data:image/png;base64,${i}`,path:s,region:h}}async function qe(e,n){const t=ft(n),a=await Ie(e,`[ -f ${t} ] && printf %s BARN-IMAGE: && base64 -w0 ${t} && echo ; true`).catch(()=>""),s=a.indexOf("BARN-IMAGE:");if(-1===s)return"";const o=a.slice(s+11).split("\n")[0].trim();if(!o)return"";const r=(n.split(".").pop()||"").toLowerCase(),i="jpg"===r||"jpeg"===r?"jpeg":"gif"===r?"gif":"webp"===r?"webp":"png";return`data:image/${i};base64,${o}`}async function Fe(e,n){if(!n)return{who:"",when:"",state:"unknown"};const t=ft(n),a=await Ie(e,[`git log -1 --format='%an%x1f%cr' -- ${t} 2>/dev/null`,'echo "--status--"',`git status --porcelain --no-renames -- ${t} 2>/dev/null`].join(" ; ")).catch(()=>""),[s="",o=""]=a.split("--status--"),[r="",i=""]=s.trim().split(""),h=o.split("\n").map(e=>e.trimEnd()).filter(Boolean)[0]?.slice(0,2)||"";let c="committed";return h.includes("?")||h.includes("A")&&!r?c="new":h.includes("D")?c="deleted":h.trim()?c="modified":r||(c="unknown"),{who:r.trim(),when:i.trim(),state:c}}async function Ge(e){const n=await Ie(e,'git branch --format="%(refname:short)" 2>/dev/null; echo ---; git rev-parse --abbrev-ref HEAD 2>/dev/null'),[t="",a=""]=n.split("---");return{current:a.trim(),branches:t.split("\n").map(e=>e.trim()).filter(Boolean)}}async function Xe(e,n){return Ie(e,`git checkout ${ft(n)} 2>&1 || git checkout -b ${ft(n)} 2>&1`)}async function Ke(e,n=50){const t=await Ie(e,`git log -n ${n} --format='%h%x1f%s%x1f%cr%x1f%an' 2>/dev/null`);return t.split("\n").map(e=>e.trim()).filter(Boolean).map(e=>{const[n,t,a,s]=e.split("");return{sha:n,subject:t,when:a,who:s}})}async function Ve(e,n){return Ie(e,`git show --patch --no-color ${ft(n)} 2>&1`)}async function Je(e,n,t){const a=(new TextEncoder).encode(t);let s="";a.forEach(e=>{s+=String.fromCharCode(e)});const o=btoa(s),r=ft(n),i=await je(e,[`mkdir -p "$(dirname ${r})"`,`printf %s ${ft(o)} | base64 -d > ${r}`,`echo "BARN-WROTE:$(wc -c < ${r})"`].join(" && "),`writing ${n}`),h=parseInt(/BARN-WROTE:\s*(\d+)/.exec(i)?.[1]||"",10);if(!Number.isFinite(h))throw new Error(`${n} was written but the pod would not say how much of it landed: ${i.trim().slice(0,200)||"no output"}`);if(h!==a.length)throw new Error(`${n} came out ${h} bytes instead of ${a.length}, so only part of it was written`)}async function ze(e,n){const t=await je(e,["git add -A || exit 1","if git diff --cached --quiet ; then echo BARN-COMMIT-NOTHING ; exit 0 ; fi",`git -c user.email=barn@rancher.local -c user.name=barn commit -q -m ${ft(n)} || exit 1`,'echo "BARN-COMMIT:$(git log -1 --format=%h)"'].join(" ; "),"the commit");if(t.includes("BARN-COMMIT-NOTHING"))throw new Error(`nothing in ${e} is uncommitted, so there was nothing to commit`);const a=/BARN-COMMIT:([0-9a-f]{7,40})/.exec(t)?.[1]||"";if(!a)throw new Error(`the commit did not happen in ${e}: ${t.trim().slice(0,200)||"git said nothing at all"}`);return a}const Ye="refs/barn/published/oci",Qe="refs/barn/published/local",Ze="refs/barn/working-build",en="refs/barn/approved";async function nn(e){return(0,s.M9)(`/v1/extensions/${(0,s.e0)(e)}/approval`).catch(()=>({sha:"",pending:[],clear:!1,read:!1}))}async function tn(e,n){if(!/^[0-9a-f]{7,40}$/i.test(n))throw new Error(`${n||"that change set"} is not a commit that can be approved`);await je(e,`git update-ref ${en} ${ft(n)}`,`approving ${n.slice(0,7)}`)}async function an(e,n){if(!/^[0-9a-f]{7,40}$/i.test(n))throw new Error(`${n||"that change set"} is not a commit that can be rejected from`);await je(e,[`git revert --no-commit ${ft(n)}..HEAD || { git revert --abort 2>/dev/null ; exit 1 ; }`,`git -c user.email=barn@rancher.local -c user.name=barn commit -q -m ${ft(`Reject the change sets after ${n.slice(0,7)}`)} || exit 1`,`git update-ref ${en} HEAD`].join("\n"),"rejecting the change sets after this one")}const sn=[`BARN_BASE=$(git rev-parse --verify -q ${Ye}`,`|| git rev-parse --verify -q ${Qe}`,`|| git rev-parse --verify -q ${en}`,"|| git rev-list --max-parents=0 HEAD 2>/dev/null | tail -1","|| git rev-parse --verify -q HEAD)"].join(" "),on="git add -A -N >/dev/null 2>&1",rn=['{ git diff --name-only --no-renames "$BARN_BASE" 2>/dev/null',"; git ls-files -o --exclude-standard 2>/dev/null ; } | sort -u | grep -c ."].join(" ");async function hn(e){const n=await Ie(e,[sn,`for r in ${Ye} ${Qe} ; do`,'git rev-parse --verify -q "$r" >/dev/null && { echo "KIND=$r"; break; } ; done','echo "SHA=$BARN_BASE"'].join("\n")).catch(()=>""),t=(/SHA=(\S+)/.exec(n)?.[1]||"").trim(),a=(/KIND=(\S+)/.exec(n)?.[1]||"").trim();return t?a===Ye?{kind:"oci",ref:a,sha:t,label:"measured against the last version that was handed over"}:a===Qe?{kind:"local",ref:a,sha:t,label:"measured against the last version published into this Rancher"}:{kind:"head",ref:"HEAD",sha:t,label:"nothing has been published yet, so this is measured against the last commit"}:{kind:"none",ref:"",sha:"",label:"this extension has no history yet, so there is nothing to measure from"}}async function cn(e,n,t){try{const a=await je(e,["test -d .git || { echo BARN-BASELINE-NOGIT ; exit 0 ; }","export GIT_INDEX_FILE=/tmp/barn-baseline-index.$$",'rm -f "$GIT_INDEX_FILE"',"git read-tree HEAD 2>/dev/null","git add -A || exit 1","tree=$(git write-tree)","unset GIT_INDEX_FILE",'[ -n "$tree" ] || exit 1',`commit=$(git -c user.email=barn@rancher.local -c user.name=barn commit-tree "$tree" -p HEAD -m ${ft(t)})`,'[ -n "$commit" ] || exit 1',`git update-ref ${n} "$commit" || exit 1`,'echo "BARN-BASELINE:$commit"'].join(" ; "),`recording the baseline ${n}`);if(a.includes("BARN-BASELINE-NOGIT"))return{sha:"",error:`${e} has no git repository in its pod, so there is no baseline to record`};const s=(/BARN-BASELINE:(\S+)/.exec(a)?.[1]||"").trim();return s?{sha:s,error:""}:{sha:"",error:`git wrote no commit for ${n}`}}catch(a){return{sha:"",error:a?.message||String(a)}}}async function dn(e){const n=await Ie(e,[sn,rn].join(" ; "));return parseInt(n.trim(),10)||0}async function ln(e){const n=await(0,s.M9)(`/v1/extensions/${(0,s.e0)(e)}/changes`).catch(()=>null);return n?.files||[]}async function un(e,n){const t=`'${n.replace(/'/g,"'\\''")}'`;return Ie(e,[sn,on,`git diff --no-renames "$BARN_BASE" -- ${t} 2>/dev/null`].join(" ; "))}function pn(e){const n=(e||"").trim();if(!/^[0-9a-f]{4,40}$/.test(n))throw new Error(`"${n}" is not a commit`);return n}function mn(e){return[`BARN_SINCE=$(git rev-parse --verify -q ${pn(e)}^{commit})`,"|| { echo BARN-NO-COMMIT ; exit 0 ; }"].join(" ")}async function fn(e,n){const t=await(0,s.M9)(`/v1/extensions/${(0,s.e0)(e)}/changes?since=${(0,s.e0)(n)}`);return t?.files||[]}async function gn(e,n,t){const a=`'${t.replace(/'/g,"'\\''")}'`,s=await Ie(e,[mn(n),on,`git diff --no-renames "$BARN_SINCE" -- ${a} 2>/dev/null`].join(" ; "));if(s.includes("BARN-NO-COMMIT"))throw new Error(`${n} is not a commit in ${e} any more, so there is nothing to measure from`);return s}async function wn(e,n=[]){const t=n.length?n.map(e=>`'${e.replace(/'/g,"'\\''")}'`).join(" "):".",a=await je(e,[`git reset -q -- ${t} 2>/dev/null || true`,`known=$(git ls-files -- ${t} 2>/dev/null)`,'[ -n "$known" ] && git checkout -- $known 2>/dev/null || true',`git clean -fd -e node_modules -- ${t} 2>/dev/null || true`,`echo "BARN-DISCARD-LEFT:$({ git diff --name-only --no-renames HEAD -- ${t} 2>/dev/null ; git ls-files -o --exclude-standard -- ${t} 2>/dev/null ; } | sort -u | tr '\\n' ' ')"`].join(" ; "),n.length?`discarding ${n.length} file${1===n.length?"":"s"}`:"discarding the working tree"),s=(/BARN-DISCARD-LEFT:(.*)/.exec(a)?.[1]||"").trim();if(s)throw new Error(`the discard did not take: ${s.split(/\s+/).slice(0,6).join(", ")} ${1===s.split(/\s+/).length?"is":"are"} still changed in ${e}`)}async function bn(e){const n=await Ie(e,['echo "BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null)"',sn,`echo "CHANGES=$(${rn})"`,'echo "LAST=$(git log -1 --format=%cI 2>/dev/null)"',"echo \"TOUCHED=$(git status --porcelain 2>/dev/null | cut -c4- | sed 's/.* -> //' | tr '\\n' '\\0' | xargs -0 -r stat -c %Y 2>/dev/null | sort -n | tail -1)\""].join(" ; ")).catch(()=>""),t=e=>{const t=new RegExp(`^${e}=(.*)$`,"m").exec(n);return(t?.[1]||"").trim()},a=parseInt(t("TOUCHED"),10);return{branch:t("BRANCH"),changes:parseInt(t("CHANGES"),10)||0,lastChange:t("LAST"),lastTouched:a?new Date(1e3*a).toISOString():""}}async function yn(e){const n=await N("configmaps",E(e)).catch(()=>null);return n?.metadata?.annotations?.[b]||""}const vn="barn.review.deferred";async function kn(e,n=""){const t=(new Date).toISOString();await Ie(e,[`git config --local ${vn} ${ft(t)}`,`git config --local ${vn}-note ${ft(n.slice(0,200))}`].join(" ; "))}async function En(e){const n=await Ie(e,[`echo "AT:$(git config --local --get ${vn} 2>/dev/null)"`,`echo "NOTE:$(git config --local --get ${vn}-note 2>/dev/null)"`].join(" ; ")).catch(()=>""),t=(/^AT:(.*)$/m.exec(n)?.[1]||"").trim();return t?{at:t,note:(/^NOTE:(.*)$/m.exec(n)?.[1]||"").trim()}:null}async function xn(e){await Ie(e,[`git config --local --unset ${vn} 2>/dev/null`,`git config --local --unset ${vn}-note 2>/dev/null`,"true"].join(" ; "))}async function Tn(e){const n={edited:"",commit:{sha:"",author:"",when:"",subject:""}};return(0,s.M9)(`/v1/extensions/${(0,s.e0)(e)}/provenance`).catch(()=>n)}const Sn="barn-snap";async function An(e,n){const t=String(Date.now()),a=n.replace(/[^\w .-]/g,"").slice(0,60)||"snapshot",s=`${Sn}/${t}`,o=await je(e,["test -d .git || { echo BARN-SNAP-NOGIT ; exit 1 ; }","idx=$(mktemp)",'cp .git/index "$idx" 2>/dev/null || true','GIT_INDEX_FILE="$idx" git add -A || { rm -f "$idx" ; exit 1 ; }','tree=$(GIT_INDEX_FILE="$idx" git write-tree)','rm -f "$idx"','[ -n "$tree" ] || { echo BARN-SNAP-NOTREE ; exit 1 ; }','parent=""','git rev-parse --verify -q HEAD >/dev/null && parent="-p HEAD"',`sha=$(git -c user.email=barn@rancher.local -c user.name=barn commit-tree $parent "$tree" -m ${ft(a)})`,'[ -n "$sha" ] || { echo BARN-SNAP-NOCOMMIT ; exit 1 ; }',`git tag -f ${s} "$sha" || exit 1`,`git rev-parse --verify -q ${s} >/dev/null || { echo BARN-SNAP-NOTAG ; exit 1 ; }`,'echo "SNAP:$sha"'].join(" ; "),`the snapshot of ${e}`),r=/SNAP:([0-9a-f]{7,40})/.exec(o);if(!r)throw new Error(`could not snapshot ${e}: ${o.trim().slice(0,200)||"no output"}`);return r[1]}async function Rn(e){const n=await Ie(e,`git log -1 --format='%H%x1f%s%x1f%cr' ${Ze} 2>/dev/null`).catch(()=>""),[t="",a="",s=""]=n.trim().split("");return t?{sha:t.trim(),subject:a.trim(),when:s.trim()}:null}async function $n(e){const n=await Ie(e,`git for-each-ref --sort=-creatordate --format='%(refname:short)%09%(creatordate:relative)%09%(contents:subject)' refs/tags/${Sn} 2>/dev/null`).catch(()=>"");return n.split("\n").map(e=>e.trimEnd()).filter(Boolean).map(e=>{const[n,t="",a=""]=e.split("\t");return{ref:n,when:t,label:a||"snapshot"}})}async function On(e,n=50){const t=await Ie(e,[`git log -n ${n} --format='%h%x1f%s%x1f%cr%x1f%an%x1f%ct' 2>/dev/null`,'echo "--snapshots--"',`git for-each-ref --sort=-creatordate --format='%(refname:short)%09%(contents:subject)%09%(creatordate:relative)%09%(creatordate:unix)' refs/tags/${Sn} 2>/dev/null`].join(" ; ")).catch(()=>""),[a="",s=""]=t.split("--snapshots--"),o=a.split("\n").map(e=>e.trim()).filter(Boolean).map(e=>{const[n,t="",a="",s="",o=""]=e.split("");return{ref:n,subject:t,when:a,who:s,kind:"commit",at:parseInt(o,10)||0}}),r=s.split("\n").map(e=>e.trim()).filter(Boolean).map(e=>{const[n,t="",a="",s=""]=e.split("\t");return{ref:n,subject:t||"snapshot",when:a,who:"",kind:"snapshot",at:parseInt(s,10)||0}});return[...o,...r].sort((e,n)=>n.at-e.at).slice(0,n)}async function Nn(e,n){const t=await je(e,`git rev-parse --verify -q ${ft(n)}^{commit} >/dev/null 2>&1 || { echo "NOREF"; exit 0; } ; git checkout ${ft(n)} -- . || exit 1 ; echo "RESTORED"`,`restoring ${n}`);if(t.includes("NOREF"))throw new Error(`could not restore ${n}: no such commit in this extension`);if(!t.includes("RESTORED"))throw new Error(`could not restore ${n}: ${t.trim().slice(0,200)||"git said nothing at all"}`)}async function _n(e){const n=await Ie(e,[sn,'test -z "$(git status --porcelain 2>/dev/null)" || exit 0',"head=$(git rev-parse --verify -q HEAD)",'[ -n "$head" ] || exit 0','[ "$head" != "$BARN_BASE" ] || exit 0','git log -1 --format=%B "$head" | grep -q "^Barn-Turn:" || exit 0','git rev-parse --verify -q "$head^" >/dev/null || exit 0','files=$(git show --name-only --format= "$head" | grep -c . )','subject=$(git log -1 --format=%s "$head")','first=$(git show --name-only --format= "$head" | head -1)','git reset --keep "$head^" >/dev/null 2>&1 || exit 0','echo "BARN-UNDID-TURN:$files:$first:$subject"'].join(" ; ")).catch(()=>""),t=/BARN-UNDID-TURN:(\d+):([^:]*):(.*)/.exec(n);if(t){const e=parseInt(t[1],10)||1,n=t[2]||"the tree",a=e>1?` and ${e-1} other file${2===e?"":"s"}`:"";return`${n}${a} (the assistant's turn "${t[3].trim()}")`}const a=await Ie(e,['f=$(git status --porcelain --no-renames 2>/dev/null | sed "s/^...//" | tr -d \'"\' | while read -r p; do [ -e "$p" ] && printf "%s\\t%s\\n" "$(stat -c %Y "$p" 2>/dev/null || echo 0)" "$p"; done | sort -rn | head -1 | cut -f2-)','[ -z "$f" ] && { echo "NONE"; exit 0; }','if git cat-file -e HEAD:"$f" 2>/dev/null; then git checkout HEAD -- "$f" 2>/dev/null; else rm -f "$f"; fi','echo "UNDID:$f"'].join(" ; ")).catch(()=>""),s=/UNDID:(.+)/.exec(a);return s?s[1].trim():""}async function In(e){return Ie(e,[sn,on,'git diff --no-renames "$BARN_BASE" 2>/dev/null'].join(" ; "))}const Pn="editor",jn=`mc-${Pn}`,Cn="/app/.queue/editor";async function Mn(e,n){const t=JSON.stringify({screen:n.screen||"",principal:n.principal||"",name:n.name||"",at:(new Date).toISOString()});await Oe(await $(e)||"",_e(`mkdir -p /app/.barn && printf %s ${ft(t)} > /app/.barn/origin`)).catch(()=>"")}async function Dn(e,n=25){const t=await(0,s.M9)(`/v1/extensions/${(0,s.e0)(e)}/turns?limit=${n}`).catch(()=>null);return t?.turns||[]}async function Bn(e){const n=e=>({available:!1,reason:e,base:"",baseRef:"",files:[]}),t=await Ie(e,["test -f /seed/barn-provenance.mjs || { echo BARN-PROV-ABSENT ; exit 0 ; }","node /seed/barn-provenance.mjs report 2>/dev/null"].join(" ; ")).catch(()=>"");if(t.includes("BARN-PROV-ABSENT"))return n("this pod was started before the Studio recorded provenance, so nothing was captured for it");const a=/BARN-PROV:(.*)/.exec(t);if(!a)return n("the pod did not answer, so what produced these lines is not known");try{return JSON.parse(a[1])}catch{return n("the pod answered with something unreadable")}}const Hn="/app/.barn/conversation-since";async function Ln(e){const n=(new Date).toISOString();return await je(e,`mkdir -p /app/.barn && printf %s ${ft(n)} > ${Hn}`,"starting a new conversation"),await Wn(e,"/clear").catch(()=>"queued"),n}async function Un(e){const n=await Ie(e,`cat ${Hn} 2>/dev/null`).catch(()=>"");return n.trim()}async function Wn(e,n,t){const a=n.replace(/\s+/g," ").trim();if(!a)throw new Error("there is nothing to ask");const s=await $(e);if(!s)throw new Error(`${e} has no running pod to ask`);t&&await Mn(e,t);const o=ft(a),r=await Ie(e,[`if ! tmux has-session -t ${jn} 2>/dev/null ; then`,`/bin/sh /seed/shell.sh ${Pn} '' '' start >/dev/null 2>&1 || true ;`,"sleep 2 ; fi ;",`if tmux has-session -t ${jn} 2>/dev/null ; then`,`tmux send-keys -t ${jn} -l ${o} && sleep 1 &&`,`tmux send-keys -t ${jn} Enter && echo BARN-ASK-SENT ;`,`else mkdir -p "$(dirname ${Cn})" && printf %s ${o} > ${Cn} &&`,"echo BARN-ASK-QUEUED ; fi"].join(" "));if(r.includes("BARN-ASK-SENT"))return"sent";if(r.includes("BARN-ASK-QUEUED"))return"queued";throw new Error(`the question did not reach ${e}: ${r.trim().slice(0,200)||"no output"}`)}const qn="BARN-LOGIN",Fn="mc-login",Gn="/app/.barn/login.log";async function Xn(e){await Ie(e,[`tmux kill-session -t ${Fn} 2>/dev/null || true`,`mkdir -p /app/.barn ; : > ${Gn}`,`tmux -f /seed/tmux.conf new-session -d -x 200 -y 50 -s ${Fn} -c /app 'claude setup-token'`,`sleep 1 ; tmux pipe-pane -t ${Fn} -o ${ft(`cat >> ${Gn}`)} 2>/dev/null || true`].join(" ; ")).catch(()=>"");for(let n=0;n<20;n++){await new Promise(e=>setTimeout(e,1500));const n=await Ie(e,`tmux capture-pane -p -t ${Fn} 2>/dev/null`).catch(()=>""),t=Kn(n);if(t)return t}throw new Error("the pod started the sign-in but printed no address to authorise at")}function Kn(e){const n=String(e||"").split("\n").map(e=>e.trim()),t=n.findIndex(e=>e.startsWith("https://"));if(-1===t)return"";let a=n[t];for(const s of n.slice(t+1)){if(!s||s.includes(" "))break;a+=s}return a}async function Vn(e){await Ie(e,[`tmux kill-session -t ${jn} 2>/dev/null || true`,`/bin/sh /seed/shell.sh ${Pn} '' '' start >/dev/null 2>&1 || true`].join(" ; ")).catch(()=>"")}function Jn(e){const n=String.fromCharCode(27),t=String(e||""),a=t.indexOf("sk-ant-");if(-1===a)return"";const s=t.slice(a,a+400).split(new RegExp(`\r?${n}\\[[0-9]*[CB]`,"g")).join("").split("\r").join("");return/^(sk-ant-[A-Za-z0-9_-]+)/.exec(s)?.[1]||""}async function zn(e,n){const t=String(n||"").trim();if(!t)throw new Error("there is no code to send");if(/\s/.test(t))throw new Error("that does not look like a code: it has whitespace in it");const a=await Ie(e,`tmux capture-pane -p -t ${Fn} 2>/dev/null || echo BARN-NO-LOGIN-SESSION`).catch(()=>"BARN-NO-LOGIN-SESSION");if(a.includes("BARN-NO-LOGIN-SESSION")){const n=await Ie(e,`cat ${Gn} 2>/dev/null`).catch(()=>""),t=Jn(n);if(t){const n=await Yn(e,t);return await Vn(e),n}}if(a.includes("BARN-NO-LOGIN-SESSION")||!a.trim())throw new Error("the sign-in this code belongs to has ended. Close this and open it again for a fresh address.");/press enter to retry/i.test(a)&&await Ie(e,[`tmux send-keys -t ${Fn} Enter`,"sleep 2"].join(" && ")).catch(()=>"");const s=await Ie(e,'node -e \'const f=process.env.HOME+"/.claude/.credentials.json";let d={};try{d=require(f)}catch{};const o=d.claudeAiOauth||{};process.stdout.write(o.accessToken&&(!o.expiresAt||o.expiresAt>Date.now())?"valid:"+(o.expiresAt||0):"no")\' 2>/dev/null').catch(()=>"no");await Ie(e,[`tmux send-keys -t ${Fn} -l ${ft(t)}`,"sleep 1",`tmux send-keys -t ${Fn} Enter`].join(" && "));const o=async n=>(await Ie(e,`tmux kill-session -t ${Fn} 2>/dev/null || true`).catch(()=>""),await Vn(e),n);for(let r=0;r<20;r++){await new Promise(e=>setTimeout(e,1500));const n=await Ie(e,`tmux capture-pane -p -t ${Fn} 2>/dev/null ; cat ${Gn} 2>/dev/null`).catch(()=>""),t=Jn(n);if(t)return o(await Yn(e,t));const a=await Ie(e,'node -e \'const f=process.env.HOME+"/.claude/.credentials.json";let d={};try{d=require(f)}catch{};const o=d.claudeAiOauth||{};process.stdout.write(o.accessToken&&(!o.expiresAt||o.expiresAt>Date.now())?"valid:"+(o.expiresAt||0):"no")\' 2>/dev/null').catch(()=>"no");if(a.startsWith("valid:")&&a.trim()!==s.trim())return o("oauth");if(/oauth error|invalid code|expired|failed/i.test(n))throw new Error("the pod did not accept that code: claude reported it invalid. A code can only be used once - take a fresh one from the address above.")}throw new Error("the pod took the code but never ended up with a working credential. The code may have been used already, or pasted incomplete - take a fresh one from the address above.")}async function Yn(e,n){const t=String(n||"").trim();if(!t)throw new Error("there is no token to set");if(/\s/.test(t))throw new Error("that does not look like a token: it has whitespace in it");const a=/^sk-ant-oat/i.test(t),s=a?"oauth":"apikey",o=a?"CLAUDE_CODE_OAUTH_TOKEN":"ANTHROPIC_API_KEY",r=['node -e \'const fs=require("fs");const p=process.env.HOME+"/.claude/settings.json";','let s={};try{s=JSON.parse(fs.readFileSync(p,"utf8"))}catch(e){};',"s.env=Object.assign({},s.env);","delete s.env.ANTHROPIC_API_KEY;delete s.env.CLAUDE_CODE_OAUTH_TOKEN;","s.env[process.argv[1]]=process.argv[2];",'fs.mkdirSync(require("path").dirname(p),{recursive:true});',`fs.writeFileSync(p,JSON.stringify(s,null,2)+"\\n",{mode:0o600})' ${ft(o)} ${ft(t)}`,"&& echo BARN-TOKEN-SET"].join(" "),i=await Ie(e,r);if(!i.includes("BARN-TOKEN-SET"))throw new Error(`the token did not reach ${e}: ${i.trim().slice(0,200)||"no output"}`);return await Vn(e),s}async function Qn(e){const n=await Ie(e,[`echo ${qn}`,'node -e \'const d=require(process.env.HOME+"/.claude/.credentials.json");const o=d.claudeAiOauth||{};if(o.accessToken&&(!o.expiresAt||o.expiresAt>Date.now()))process.stdout.write("credentials")\' 2>/dev/null','[ -n "$ANTHROPIC_API_KEY$CLAUDE_CODE_OAUTH_TOKEN" ] && echo apikey','grep -q \'"\\(ANTHROPIC_API_KEY\\|CLAUDE_CODE_OAUTH_TOKEN\\)"\' "$HOME/.claude/settings.json" 2>/dev/null && echo apikey','grep -o \'"emailAddress":"[^"]*"\' "$HOME/.claude.json" 2>/dev/null | head -1'].join(" ; ")).catch(()=>"");if(!n.includes(qn))return{read:!1,signedIn:!1,account:""};const t=/"emailAddress":"([^"]*)"/.exec(n);return{read:!0,signedIn:n.includes("credentials")||n.includes("apikey")||!!t,account:t?t[1]:""}}async function Zn(e,n,t){if(!/^[\w.-]+\/[\w.-]+$/.test(n))throw new Error(`"${n}" is not owner/name`);if(!t)throw new Error("the extension is not on a branch");const a=await et(e,"GET",`/repos/${n}/pulls?state=open&per_page=100`),s=Array.isArray(a)?a:[],o=s.find(e=>e?.head?.ref===t);return o?{number:o.number,title:o.title,url:o.html_url,head:o.head.ref}:null}async function et(e,n,t,a){const s=await Be(e,[`node -e ${ft(K())}`,ft(n),ft(t),ft(void 0===a?"":JSON.stringify(a)),"2>&1"].join(" "));return V(s,`${e}'s pod`).body}async function nt(e,n){const t=await W();if(!t)throw new Error("no extension pod is running to ask GitHub from");const a=await Oe(t,_e(`node -e ${ft(K())} ${ft(e)} ${ft(n)} '' 2>&1`));return V(a,"a running extension pod")}async function tt(){if(!(await Ot("")).hasToken)return null;const e=await nt("GET","/user");return{login:e?.body?.login||"",scopes:String(e?.scopes||"").split(",").map(e=>e.trim()).filter(Boolean),expiresAt:e?.expires||""}}async function at(e=30){const n=Math.max(1,Math.min(100,e)),t=await nt("GET",`/user/repos?sort=updated&per_page=${n}&affiliation=owner,collaborator,organization_member`),a=Array.isArray(t?.body)?t.body:[];return a.map(e=>({fullName:e.full_name||"",private:!!e.private,updatedAt:e.updated_at||"",defaultBranch:e.default_branch||"",description:e.description||""})).filter(e=>!!e.fullName)}async function st(e,n){if(!/^[\w.-]+\/[\w.-]+$/.test(n))throw new Error(`"${n}" is not owner/name`);const t=await et(e,"GET",`/repos/${n}`);return t?.default_branch||"main"}async function ot(e,n,t){if(!/^[\w.-]+\/[\w.-]+$/.test(n))throw new Error(`"${n}" is not owner/name`);try{const a=await et(e,"POST",`/repos/${n}/pulls`,t);return{number:a.number,title:a.title,url:a.html_url,head:a.head?.ref||t.head}}catch(a){const s=await Zn(e,n,t.head).catch(()=>null);if(s)return s;throw a}}async function rt(e,n,t,a){await et(e,"POST",`/repos/${n}/issues/${t}/comments`,{body:a})}async function it(e,n){const t=await dt().catch(()=>"");if(!t)return;const a=ft(n||"/");await Ie(e,[`printf %s ${ft(t)} > "$HOME/.capture-token"`,'chmod 600 "$HOME/.capture-token"',`node -e 'const fs=require("fs");const p=process.env.HOME+"/.claude/settings.json";let s={};try{s=JSON.parse(fs.readFileSync(p,"utf8"))}catch(e){};s.env=Object.assign({},s.env,{BARN_CAPTURE_ROUTE:process.argv[1]});fs.mkdirSync(require("path").dirname(p),{recursive:true});fs.writeFileSync(p,JSON.stringify(s,null,2)+"\\n",{mode:0o600})' ${a}`].join(" ; ")).catch(()=>"")}function ht(e){const n=new Set;for(const t of String(e||"").split("\n")){if(!t.startsWith("+")||t.startsWith("+++"))continue;for(const a of t.matchAll(/\s([a-zA-Z][a-zA-Z0-9-]*)="([^"]+)"/g)){const[,e,t]=a;if(!t.includes("{")&&ct.test(e))if("class"===e){const e=t.trim().split(/\s+/)[0];e&&n.add(`.${e}`)}else"id"===e?n.add(`#${t}`):n.add(`[${e}="${t}"]`)}const e=/^\+\s*([.#][\w-][\w-]*(?:[\s>+~][^{]*)?)\s*\{/.exec(t);if(e){const t=e[1].trim().split(/[\s>+~,]/)[0];t&&t.length>1&&n.add(t)}if(n.size>=6)break}return[...n]}const ct=/^(class|id|data-[\w-]+|name|role|aria-label|href|src|alt|title|type|placeholder|for)$/;async function dt(){const e=await(0,a.c)("/v3/tokens",{method:"POST",body:JSON.stringify({type:"token",description:"barn: change-set screenshot",ttl:6e5})}).catch(()=>null);return e?.token||""}async function lt(e,n,t,a=[]){const s=/^[0-9a-f]{7,40}$/i.test(n)?n:"";if(!s)return{before:"",after:"",regions:[],geometry:0,width:0,height:0,why:`${n||"that change set"} is not a commit this can shoot`};const o=`/app/.shots/${s}-before.png`,r=`/app/.shots/${s}-after.png`,i=await Ie(e,[`[ -f ${o} ] && printf %s BARN-BEFORE: && base64 -w0 ${o} && echo ;`,`[ -f ${r} ] && printf %s BARN-AFTER: && base64 -w0 ${r} && echo ;`,`[ -f ${r}.json ] && printf %s BARN-REGIONS: && base64 -w0 ${r}.json && echo ;`,"true"].join(" ")).catch(()=>""),h=e=>{const n=i.indexOf(e);if(-1===n)return"";const t=i.slice(n+e.length).split("\n")[0].trim();return t?`data:image/png;base64,${t}`:""},c=e=>{const n=e.indexOf("BARN-REGIONS:");if(-1===n)return{regions:[],width:0,height:0,geometry:0};try{const t=e.slice(n+13).split("\n")[0].trim(),a=JSON.parse(atob(t));return{regions:Array.isArray(a?.regions)?a.regions:[],width:Number(a?.width)||0,height:Number(a?.height)||0,geometry:Number(a?.v)||0}}catch{return{regions:[],width:0,height:0,geometry:0}}},d=h("BARN-BEFORE:"),l=h("BARN-AFTER:"),u=c(i);if(d&&l)return{before:d,after:l,regions:u.regions,geometry:u.geometry,width:u.width,height:u.height,why:""};const p=(t||"/").replace(/[^\w]+/g,"-").replace(/^-|-$/g,"")||"root",m=`/app/.shots/${s}-${p}.png`,f=await dt().catch(()=>""),g=a.length?"":await Ie(e,`git show --unified=0 --no-color ${s} 2>/dev/null`).catch(()=>""),w=a.length?a:ht(g),b=w.map(e=>`--note ${ft(`${e}=changed here`)}`).join(" "),y=`${m}.err`,v="/seed/skills__barn-screenshot__screenshot.mjs",k=await Ie(e,[`if [ ! -f ${m} ] ; then`,'mkdir -p "$HOME/.claude/skills/barn-screenshot" ;',`[ -f ${v} ] && cp ${v} "$HOME/.claude/skills/barn-screenshot/screenshot.mjs" ;`,'node "$HOME/.claude/skills/barn-screenshot/screenshot.mjs"',`--path ${ft(t||"/")} ${b}`,f?`--token ${ft(f)}`:"",`--output ${m} >${y} 2>&1 || true ; fi ;`,`[ -f ${m} ] && echo BARN-SHOT: && base64 -w0 ${m} && echo ;`,`[ ! -f ${m} ] && [ -f ${y} ] && printf %s BARN-SHOT-ERR: && tail -c 600 ${y} ;`,"true"].filter(Boolean).join(" ")).catch(e=>`ERR:${e?.message||e}`);if(k.startsWith("ERR:"))return{before:"",after:"",regions:[],geometry:0,width:0,height:0,why:k.slice(4).trim()};const E=k.indexOf("BARN-SHOT:"),x=-1===E?"":k.slice(E+10).split("\n")[0].trim();if(!x){const e=k.indexOf("BARN-SHOT-ERR:"),n=-1===e?"":k.slice(e+14).trim();return{before:"",after:"",regions:[],geometry:0,width:0,height:0,why:n?`The pod could not take this picture. It said: ${n}`:"The pod took no picture and said nothing about why, which usually means its browser could not be reached at all."}}return{before:"",after:`data:image/png;base64,${x}`,regions:[],geometry:0,width:0,height:0,why:l||d?"only one of the pair was recorded for this change set":"this change set was committed before the pod started taking pictures, so there is no Before for it - this is the page as it stands now"}}const ut=4096;async function pt(e,n,t){const a=await $(e);if(!a)throw new Error(`${e} has no running pod to write to`);return mt(a,n,t,e)}async function mt(e,n,t,a="the pod",s){const o=new Uint8Array(t);let r="";for(let l=0;l<o.length;l+=8192)r+=String.fromCharCode(...o.subarray(l,l+8192));const i=btoa(r),h=ft(n),c=`${h}.b64`;await Oe(e,_e(`mkdir -p "$(dirname ${h})" && : > ${c}`),void 0,s);for(let l=0;l<i.length;l+=ut){const n=i.slice(l,l+ut);await Oe(e,_e(`printf %s '${n}' >> ${c}`),void 0,s)}const d=await Oe(e,_e(`base64 -d ${c} > ${h} && rm -f ${c} && wc -c < ${h}`),void 0,s);if(!parseInt(d.trim(),10))throw new Error(`the image did not land in ${a}`)}function ft(e){return`'${e.split("'").join("'\\''")}'`}const gt="cattle-ui-plugin-system",wt="plugins";async function bt(e){const n=await Ue(e,"package.json");try{const e=JSON.parse(n);return{name:e.name,version:e.version,annotations:e.rancher?.annotations||{}}}catch{throw new Error(`could not read the package.json of ${e}: ${n.trim()||"no output"}`)}}const yt="settings",vt="gh_token",kt="barn.rancher.io/gh-token";function Et(e){return e?`barn.rancher.io/repo.${e}`:""}function xt(e){return`gh_repo.${e}`}function Tt(e){return btoa(String.fromCharCode(...(new TextEncoder).encode(e)))}const St=`${f}/api/v1/namespaces/${d}/secrets/${yt}`,At={Accept:"application/json;as=PartialObjectMetadata;g=meta.k8s.io;v=v1"};async function Rt(){const e=await(0,a.c)(St,{headers:At}).catch(()=>null);return e?.metadata||null}function $t(e){const n=new Set;return(e?.managedFields||[]).forEach(e=>{Object.keys(e?.fieldsV1?.["f:data"]||{}).forEach(e=>{e.startsWith("f:")&&n.add(e.slice(2))})}),[...n]}async function Ot(e){const n=await Rt(),t=n?.annotations||{},a=$t(n),s=Et(e),o=s&&t[s]||"";return{hasToken:"set"===t[kt]||a.includes(vt),repo:o,legacyRepo:!o&&!!e&&a.includes(xt(e))}}async function Nt(e,n){const t={},s={};void 0!==n.token&&(t[vt]=""===n.token?null:Tt(n.token),s[kt]=""===n.token?null:"set");const o=Et(e);if(void 0!==n.repo&&o&&(s[o]=""===n.repo?null:n.repo,t[xt(e)]=null),!Object.keys(t).length&&!Object.keys(s).length)return;await he();const r=()=>(0,a.c)(St,{method:"PATCH",headers:{"Content-Type":"application/merge-patch+json",...At},body:JSON.stringify({metadata:{annotations:s},data:t})});try{await r()}catch(i){if(!/404|not ?found/i.test(i?.message||""))throw i;await(0,a.c)(`${f}/api/v1/namespaces/${d}/secrets`,{method:"POST",headers:At,body:JSON.stringify({apiVersion:"v1",kind:"Secret",type:"Opaque",metadata:{namespace:d,name:yt}})}).catch(e=>{if(!/409|already exists|alreadyexists/i.test(e?.message||""))throw e}),await r()}}const _t=["Reading the package","Building the extension","Serving it from the pod","Installing it into Rancher"];class It extends Error{constructor(e,n,t){super(e),r(this,"log",void 0),r(this,"stage",void 0),this.name="PublishError",this.stage=n,this.log=t}}async function Pt(e,n){const t=_t.length,a=e=>n?.(e,_t[e-1],t),s=await $(e);if(!s)throw new It(`${e} has no running pod to build in`,_t[0],"");const o=await nn(e).catch(()=>({sha:"",pending:[],clear:!1,read:!1}));if(!o.clear){const e=o.pending.length;throw new It(o.read?`${e} change set${1===e?"":"s"} ${1===e?"has":"have"} not been reviewed yet. Approve ${1===e?"it":"them"} on the Changes tab, or reject ${1===e?"it":"them"}, and publish again.`:"This could not read what has been reviewed, so it will not publish. Open the Changes tab and see whether it can.",_t[0],"")}a(1);const{name:r,version:i,annotations:h}=await bt(e),c=`${r}-${i}`;a(2);const d=await Oe(s,_e(`cd /app && ./node_modules/@rancher/shell/scripts/build-pkg.sh ${r} 2>&1`)),l=`dist-pkg/${c}/${c}.umd.min.js`,u=await Oe(s,_e(`test -f /app/${l} && echo yes`));if(!u.includes("yes"))throw new It(`${r} did not build`,_t[1],d);a(3);const p=await Oe(s,_e(["cd /app",`mkdir -p public/${wt}`,`rm -rf public/${wt}/${c}`,`cp -r dist-pkg/${c} public/${wt}/${c}`,"echo BARN-COPY-OK"].join(" && ")));if(!p.includes("BARN-COPY-OK"))throw new It("the built bundle could not be copied where the pod serves it",_t[2],`${d}\n${p}`);a(4);const m=(await Oe(s,_e("date +%s"))).trim(),f=`${S(e)}/${wt}/${c}/${c}.umd.min.js?t=${m}`;try{await Mt(r,i,f,h)}catch(y){throw new It(y?.message||String(y),_t[3],d)}const g=await cn(e,Qe,`Published ${r} ${i} into this Rancher`),w=await cn(e,Ze,`Working build ${r} ${i}`),b=[g.error?`\n[barn] the publish worked, but the baseline ref could not be written, so the review screens will still measure from the previous point: ${g.error}`:"",w.error?`\n[barn] the publish worked, but the working-build ref could not be written, so a later failure will not have this build to roll back to: ${w.error}`:""].join("");return{plugin:r,version:i,url:f,log:`${d}${b}`}}const jt=["Reading the settings","Reading the package","Committing the package","Pushing to GitHub"];async function Ct(e,n,t,a,s="HEAD"){const o=jt.length,r=e=>t?.(e,jt[e-1],o);if(r(1),!n)throw new It("No repository was given.",jt[0],"");if(!(await Ot("")).hasToken)throw new It("No GitHub token is configured. Add one in the editor settings.",jt[0],"");if(!/^[\w.-]+\/[\w.-]+$/.test(n))throw new It(`"${n}" is not owner/name`,jt[0],"");const i=await nn(e).catch(()=>({sha:"",pending:[],clear:!1,read:!1}));if(!i.clear){const e=i.pending.length;throw new It(`${e} change set${1===e?"":"s"} ${1===e?"has":"have"} not been reviewed yet. Approve ${1===e?"it":"them"} on the Changes tab before handing this over.`,jt[0],"")}if(!a)throw new It("A push has to say which branch it is going to. Handing a change over for review is handOverForReview(); putting a reviewed change on the default branch is distributeExtension(), which will not run until both sign-offs are in.",jt[0],"");if(!/^[\w./-]+$/.test(a))throw new It(`"${a}" is not a branch name`,jt[0],"");await Nt(e,{repo:n}).catch(()=>null);const h=await $(e);if(!h)throw new It(`${e} has no running pod to push from`,jt[0],"");r(2);const{name:c,version:d}=await bt(e),l=`https://github.com/${n}.git`;if(await De(e),r(3),"HEAD"===s){const n=await Ie(e,["git add -A",`git diff --cached --quiet || git -c user.email=barn@rancher.local -c user.name=barn commit -q -m ${ft(`${c} ${d}`)}`,"echo BARN-COMMIT-OK"].join(" ; "));if(!n.includes("BARN-COMMIT-OK"))throw new It("the package could not be committed",jt[2],X(n))}r(4);const u=await Ie(e,[F(),'BARN_GH_HEADER="AUTHORIZATION: basic $(printf %s "x-access-token:$BARN_GH_TOKEN" | base64 -w0)"',"export BARN_GH_HEADER",`git --config-env=http.extraheader=BARN_GH_HEADER push ${ft(l)} ${ft(s)}:refs/heads/${a} 2>&1 || exit 1`,"echo BARN-PUSH-OK"].join(" ; ")),p=/BARN-NO-TOKEN:(\d+)/.exec(u);if(p)throw new It(`The push did not happen: ${G(p[1])}.`,jt[3],"");if(!u.includes("BARN-PUSH-OK"))throw new It(`could not push to ${n}`,jt[3],X(u));return{plugin:c,version:d,repo:n,branch:a,url:`https://github.com/${n}/tree/${a}`,log:X(u)}}async function Mt(e,n,t,s){const o={apiVersion:"catalog.cattle.io/v1",kind:"UIPlugin",metadata:{namespace:gt,name:e},spec:{plugin:{name:e,version:n,endpoint:t,noCache:!0,metadata:{...s,direct:"true"}}}},r="catalog.cattle.io.uiplugins",i=await(0,a.c)(`${f}/v1/${r}/${gt}/${e}`).catch(()=>null);i?await(0,a.c)(`${f}/v1/${r}/${gt}/${e}`,{method:"PUT",body:JSON.stringify({...i,spec:o.spec})}):await(0,a.c)(`${f}/v1/${r}`,{method:"POST",body:JSON.stringify(o)})}async function Dt(e){const{name:n}=await bt(e),t="catalog.cattle.io.uiplugins",s=await(0,a.c)(`${f}/v1/${t}/${gt}/${n}`).catch(()=>null);if(!s)throw new Error(`${n} is not installed in this Rancher`);return await(0,a.c)(`${f}/v1/${t}/${gt}/${n}`,{method:"DELETE"}),n}async function Bt(e){const n=await bt(e).then(e=>e.name).catch(()=>"");if(!n)return"";const t=await(0,a.c)(`${f}/v1/catalog.cattle.io.uiplugins/${gt}/${n}`).catch(()=>null);return t?.spec?.plugin?.version||""}async function Ht(e,n){const t=/^(escape|esc|enter|return|tab|space)$/i.test(n),a=t?`tmux send-keys -t ${jn} ${ft(Lt(n))}`:`tmux send-keys -t ${jn} -l ${ft(n)} && sleep 1 && tmux send-keys -t ${jn} Enter`,s=await Ie(e,[`if tmux has-session -t ${jn} 2>/dev/null ; then`,`${a} && echo BARN-ANSWERED ;`,"else echo BARN-NO-SESSION ; fi"].join(" "));if(!s.includes("BARN-ANSWERED"))throw new Error(`the answer did not reach ${e}: ${s.trim().slice(0,200)||"no output"}`)}function Lt(e){return{esc:"Escape",escape:"Escape",enter:"Enter",return:"Enter",tab:"Tab",space:"Space"}[e.toLowerCase()]||e}async function Ut(e,n=20){const t=await(0,s.M9)(`/v1/extensions/${(0,s.e0)(e)}/pane?lines=${Math.max(4,Math.min(60,n))}`).catch(()=>null);return t?.text||""}async function Wt(){const e=await(0,a.c)("/v1/management.cattle.io.clusters").catch(()=>null);if(!e)throw new Error("Rancher would not list its clusters");return(e.data||[]).map(e=>{const n=e?.metadata?.name||e?.id||"",t=(e?.status?.conditions||[]).some(e=>"Ready"===e?.type&&"True"===e?.status);return n?{id:n,name:e?.spec?.displayName||n,ready:t,provider:e?.status?.provider||e?.status?.driver||"",version:e?.status?.version?.gitVersion||"",nodes:Number(e?.status?.nodeCount)||0}:null}).filter(Boolean).sort((e,n)=>e.name.localeCompare(n.name))}async function qt(e){const n=await Ie(e,[`if tmux has-session -t ${jn} 2>/dev/null ; then`,`tmux send-keys -t ${jn} Escape && echo BARN-STOP-SENT ;`,"else echo BARN-STOP-NONE ; fi"].join(" "));if(n.includes("BARN-STOP-SENT"))return"sent";if(n.includes("BARN-STOP-NONE"))return"none";throw new Error(`the interrupt did not reach ${e}: ${n.trim().slice(0,200)||"no output"}`)}const Ft="BARN-CLAUDE:",Gt=String.raw`
